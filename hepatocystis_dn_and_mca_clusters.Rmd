...
This R Markdown file contains the R code for generating Figure S11 from the manuscript on the draft genome of Hepatocystis sp. ex Piliocolobus tephrosceles.
...
ggplot(data=counts_df, aes(x=mca_cluster, y=observed_expected_ratio)) + geom_bar(stat="identity") + xlab("Malaria Cell Atlas cluster") +
  ylab("Ratio of observed vs expected nr of genes with dN rank in top 5%") + theme(plot.title = element_text(hjust = 0.5)) + ggtitle("Genes with dN in the top 5%: observed versus expected ratios for MCA clusters")
```
